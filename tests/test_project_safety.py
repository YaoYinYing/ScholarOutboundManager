...
def test_readme_exists_and_documents_current_cli_chain() -> None:
    ...
    assert "overview" in readme_text.lower() and "next safe action" in readme_text.lower()
    ...
    assert "overview | candidates | activate | status | logs | settings" in readme_text.lower()
    ...
    assert "`overview | candidates | activate | status | logs | settings`" in readme_text.lower()
    assert "the `candidates` page presents redacted route navigation and choosing" in readme_text.lower()
    assert "`settings` groups allowlisted field editing by purpose" in readme_text.lower()
    assert "`logs` is the only top-level page that shows raw command previews" in readme_text.lower()
