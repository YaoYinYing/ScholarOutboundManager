...
def test_readme_exists_and_documents_current_cli_chain() -> None:
    ...
    assert "transactional config-editing foundation plus workflow previews" in readme_text.lower()
    assert "workflow control plane layered over the existing cli and helper operations" in readme_text.lower()
    ...
    assert "dashboard | config | fetch & probe | artifacts | selection | sidecar | pool | troubleshooting | snippets" in readme_text.lower()
    ...
    assert "selection tab supports redacted candidate navigation and choosing" in readme_text.lower()
    assert "config tab supports allowlisted field editing only" in readme_text.lower()
    assert "recent operation history is shown from a redacted action journal" in readme_text.lower()
    assert "network and systemd actions still require confirmation before side effects" in readme_text.lower()
    ...
    assert "hysteria2 remains experimental and disabled by default in the tui" in readme_text.lower()
    ...
    assert "the tui never modifies production xray, xrayr, or `x-ui` directly" in readme_text.lower()
