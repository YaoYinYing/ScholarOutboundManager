...
def test_readme_exists_and_documents_current_cli_chain() -> None:
    ...
    assert "scholar-outbound-manager tui" in readme_text.lower()
    assert "scholar-outbound-manager tui config.yaml" in readme_text.lower()
    ...
    assert "home" in readme_text.lower() and "next recommended action" in readme_text.lower()
    ...
    assert "config-centered around `config.yaml`" in readme_text.lower()
    assert "first-run wizard/template flow" in readme_text.lower()
    assert "artifact paths are derived from `user_data_dir`" in readme_text.lower()
