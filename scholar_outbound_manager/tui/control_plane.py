...
def load_control_plane_state(
    *,
    ...
    fetch_policy = get_action_policy("fetch")
    probe_policy = get_action_policy("probe")
    artifact_policy = get_action_policy("artifact_check")
    select_policy = get_action_policy("select")
    stage_policy = get_action_policy("sidecar_stage")
    pool_policy = get_action_policy("pool_stage")
    restart_policy = get_action_policy("service_restart")
    validate_policy = get_action_policy("service_validate")
    ...
    command_state = CommandState(
        ...
        operations=[
            OperationSpec(
                ...
                requires_confirmation=fetch_policy.requires_confirmation,
                network_access=fetch_policy.network_access,
                systemd_access=fetch_policy.systemd_access,
                sensitive_outputs=fetch_policy.writes_artifact,
                ...
                risk_note=fetch_policy.user_facing_risk,
            ),
            OperationSpec(
                key="probe",
                title="Probe Candidates",
                command=probe_command,
                requires_confirmation=probe_policy.requires_confirmation,
                network_access=probe_policy.network_access,
                systemd_access=probe_policy.systemd_access,
                sensitive_outputs=probe_policy.writes_artifact,
                ...
                risk_note=probe_policy.user_facing_risk,
            ),
            OperationSpec(
                key="artifact_check",
                title="Check Artifact Lineage",
                command=artifact_check_command,
                requires_confirmation=artifact_policy.requires_confirmation,
                network_access=artifact_policy.network_access,
                systemd_access=artifact_policy.systemd_access,
                sensitive_outputs=artifact_policy.writes_artifact,
                expected_artifacts=[],
                timeout_seconds=30.0,
                description="Check candidates, probe summary, and passed candidates for lineage consistency.",
                risk_note=artifact_policy.user_facing_risk,
            ),
            OperationSpec(
                key="select",
                title="Select Candidate",
                command=select_command,
                requires_confirmation=select_policy.requires_confirmation,
                network_access=select_policy.network_access,
                systemd_access=select_policy.systemd_access,
                sensitive_outputs=select_policy.writes_artifact,
                ...
                risk_note=select_policy.user_facing_risk,
            ),
            OperationSpec(
                key="sidecar_stage",
                title="Stage Sidecar",
                command=sidecar_stage_command,
                requires_confirmation=stage_policy.requires_confirmation,
                network_access=stage_policy.network_access,
                systemd_access=stage_policy.systemd_access,
                sensitive_outputs=stage_policy.writes_artifact,
                expected_artifacts=[selected_candidate_path],
                timeout_seconds=120.0,
                description="Prepare the managed sidecar runtime without touching production Xray or XrayR.",
                risk_note=stage_policy.user_facing_risk,
            ),
            OperationSpec(
                key="pool_stage",
                title="Stage Sidecar Pool",
                command=pool_stage_command,
                requires_confirmation=pool_policy.requires_confirmation,
                network_access=pool_policy.network_access,
                systemd_access=pool_policy.systemd_access,
                sensitive_outputs=pool_policy.writes_artifact,
                expected_artifacts=[pool_plan_path],
                timeout_seconds=120.0,
                description="Prepare the managed multi-port sidecar pool plan and runtime files.",
                risk_note=pool_policy.user_facing_risk,
            ),
            OperationSpec(
                key="service_restart",
                title="Restart Sidecar Service",
                command=service_restart_command,
                requires_confirmation=restart_policy.requires_confirmation,
                network_access=restart_policy.network_access,
                systemd_access=restart_policy.systemd_access,
                sensitive_outputs=restart_policy.writes_artifact,
                expected_artifacts=[],
                timeout_seconds=60.0,
                description="Restart only the ScholarOutboundManager-managed sidecar service.",
                risk_note=restart_policy.user_facing_risk,
            ),
            OperationSpec(
                key="service_validate",
                title="Validate Sidecar Service",
                command=service_validate_command,
                requires_confirmation=validate_policy.requires_confirmation,
                network_access=validate_policy.network_access,
                systemd_access=validate_policy.systemd_access,
                sensitive_outputs=validate_policy.writes_artifact,
                expected_artifacts=[],
                timeout_seconds=60.0,
                description="Validate the managed sidecar service with explicit runtime and network checks.",
                risk_note=validate_policy.user_facing_risk,
            ),
            OperationSpec(
                key="snippet",
